...
function homeo_property_display_author($post, $display_type = 'logo', $echo = true) {
	...
	if ( $author_id ) {
		...
		if ( WP_RealEstate_User::is_agent($author_id) ) {
			...
			$logo = homeo_get_avatar( $author_id, 80 );
...
function homeo_property_display_custom_field_meta($post, $meta_key, $icon = '', $title = '', $suffix = '', $echo = false) {
	$obj_property_meta = WP_RealEstate_Property_Meta::get_instance($post->ID);

	$value = $obj_property_meta->get_custom_post_meta( $meta_key );
