			...
			<div class="row">
					...
					<div class="user_tab_menu">
						<div class="user_dashboard_links">
							...
							<ul class="list-group">
								...
								<li class="list-group-item"><a href="<?php echo esc_url( $prop_invoice_link ); ?>" class="d-block user_tab_active"><i class="fa fa-file-text-o"></i><?php esc_html_e('My Invoices', 'zoacres'); ?></a></li>
				...
				<div class="col-md-9">
					<div class="col-md-12 top_dahsboard_wrapper dashboard_package_row">
						...
						<h3 class="entry-title"><?php esc_html_e('Invoice Lists', 'zoacres'); ?></h3>
						
						<div class="zoacres-invoices-wrap">
						<?php
							$author_id = $current_user->ID;
							
							$invoices = get_user_meta( $author_id, 'zoacres_package_invoices', true );
							//print_r( $invoices );
							...
							?>
								<table class="table">
									<thead>
										<tr>
											<th scope="col"><?php esc_html_e('Transaction ID', 'zoacres'); ?></th>
											<th scope="col"><?php esc_html_e('Pack ID', 'zoacres'); ?></th>
											<th scope="col"><?php esc_html_e('Pay By', 'zoacres'); ?></th>
											<th scope="col"><?php esc_html_e('Amount', 'zoacres'); ?></th>
											<th scope="col"><?php esc_html_e('Date', 'zoacres'); ?></th>
										</tr>
									</thead>
									<tbody>
									<?php
										foreach( $invoices as $invoice_time => $invoice ){
											
											$transaction_id = isset( $invoice['transaction_id'] ) ? $invoice['transaction_id'] : '';
											$pack_id = isset( $invoice['pack_id'] ) ? $invoice['pack_id'] : '';
											$pay_by = isset( $invoice['pay_by'] ) ? $invoice['pay_by'] : '';
											$transaction_amount = isset( $invoice['transaction_amount'] ) ? $invoice['transaction_amount'] : '';
											$transaction_date = isset( $invoice['transaction_datetime'] ) ? $invoice['transaction_datetime'] : '';
											
											if( $transaction_id ):
										?>
												<tr>
													<th scope="row"><?php echo esc_html( $transaction_id ); ?></th>
													<td><?php echo esc_html( $pack_id ); ?></td>
													<td><?php echo esc_html( $pay_by ); ?></td>
													<td><?php echo esc_html( $transaction_amount ); ?></td>
													<td><?php $date = new DateTime(); $date->setTimestamp($transaction_date); echo esc_html( $date->format('Y/m/d h:i:sa') ); ?></td>
												</tr>
