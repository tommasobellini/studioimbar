...
function wpestate_estate_property_design_agent_contact($attributes,$content = null){
    ...
    if ( $contact_form_7_agent==''){ 
        ...
        <textarea id="agent_comment" name="comment" class="form-control" cols="45" rows="8" aria-required="true">'.__("I'm interested in ","wpestate").'[ '.get_the_title($propid).' ]</textarea>
...
function wpestate_property_sh_download_pdf($prop_id){
    ...
    if ($attachments) {
        ...
        $return_string.= '<div class="download_docs">'.__('Documents','wpestate').'</div>';
...
function wpestate_property_page_design_acc($attributes,$content = null){
    ...
    extract(shortcode_atts(array(
            ...
            'yelp_details'          => __("Yelp Details","wpestate"),
...
function wpestate_shortcode_multi_units($propid,$property_subunits_master,$is_print=0){
    ...
    if( $display==1 ){
        ...
        if( intval($property_subunits_master)!=0 && $property_subunits_master!=$propid){
            ...
            echo ' <a href="'.get_permalink($property_subunits_master).'" target="_blank">'.get_the_title($property_subunits_master).'</a>'; 
            print'</h4>';
            ...
            if(is_array($property_subunits_list)){
                foreach($property_subunits_list as $prop_id_unit){
                    ...
                    if($prop_id!=$prop_id_unit && $status=='publish'){
                        ...
                        $property_rooms         =   get_post_meta($prop_id_unit, 'property_bedrooms', true);
                        ...
                        $link                   =   get_permalink($prop_id_unit);
                        ...
                        if($is_print!=1){
                            ...
                            print '<div class="subunit_details">';
                                ...
                                print '<div class="subunit_price">'; wpestate_show_price($prop_id_unit,$currency,$where_currency);
                                print '</div></div>';
